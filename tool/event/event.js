/************************************ 事件 ********************************************/
/* 注意：不能写成：
YYC.Tool.event.addEvent(document.getElementById("test_div"), "mousedown", YYC.Tool.event.bindEvent(this, Handler));
YYC.Tool.event.removeEvent(document.getElementById("test_div"), "mousedown", YYC.Tool.event.bindEvent(this, Handler));

这样不能移除掉绑定的事件！因为YYC.Tool.event.bindEvent(this, Handler)不是同一个函数！（因为bindEvent返回了一个匿名函数，这两个
bindEvent返回的匿名函数不是同一个！）

应该写为：
this._Handle = YYC.Tool.event.bindEvent(this, Handler);
YYC.Tool.event.addEvent($("div"), "mousedown", _Handle);
YYC.Tool.event.removeEvent($("div"), "mousedown", _Handle);

这样_Handle就是同一个函数了！
*/
YYC.namespace("Tool").event = (function(){
    return {
